...
#[cfg(wifi_credentials)]
mod wifi_credentials;
#[cfg(wifi_credentials)]
use wifi_credentials as credentials;

#[cfg(not(wifi_credentials))]
mod default_wifi_credentials;
#[cfg(not(wifi_credentials))]
use default_wifi_credentials as credentials;
...
fn mainfunc() -> Result<()> {
    ...
    //check id 
    let res_id = wifi_abstraction::http_post(&(credentials::get_server()+ "api/id/check"), &[("DisplayID", &id.to_string())], false)?;
    if res_id[0] == '0' as u8 {
        let res_new_id = wifi_abstraction::http_post(&(credentials::get_server() + "api/id/new"), &[], false)?;
        id = String::from_utf8(res_new_id)?.parse::<u32>()?;
        id_nvs.set_u32("ID", id)?;
    }
    drop(res_id);

    let res_sleep_time = wifi_abstraction::http_post(&(credentials::get_server() + "api/sleep/requestTime"), &[("DisplayID", &id.to_string())], false)?;
    let sleep_time = String::from_utf8(res_sleep_time)?.parse::<u64>()?;
    unsafe{esp_sleep_enable_timer_wakeup(sleep_time * 1000000);}

    let res_update = wifi_abstraction::http_post(&(credentials::get_server() + "api/update/necessary"), &[("DisplayID", &id.to_string())], false)?;
    ...
    let res = wifi_abstraction::http_post(&(credentials::get_server() + "api/btmp/full"), &[("DisplayID", &id.to_string())], true)?;
    //delete wifi
    wifi.disconnect()?;
    wifi.stop()?;
